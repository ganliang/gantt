...
Gantt.plugin(function (t) {
	...
	t.attachEvent("onGanttReady", function () {
		function e(e) {
			...
			if (!(+e.start_date > +n.max_date || +e.end_date && +e.end_date < +n.min_date || +e.start_date < +n.min_date)) {
				...
				a.setAttribute("marker_id", e.id);
				a.setAttribute("id", e.id);
				var i = "gantt_marker";
				t.templates.marker_class && (i += " " + t.templates.marker_class(e)),
				e.css && (i += " " + e.css),
				e.title && (a.title = e.title),
				a.className = i;
				var s = t.posFromDate(e.start_date);
				if (a.style.left = s + "px", a.style.height = Math.max(t._y_from_ind(t._order.length), 0) + "px", e.end_date) {
					var r = t.posFromDate(e.end_date);
					a.style.width = Math.max(r - s, 0) + "px"
				}
				return e.text && (a.innerHTML = "<div class='gantt_marker_content' >" + e.text + "</div>"),
				a
			}
		}
		var n = document.createElement("div");
		n.className = "gantt_marker_area",
		t.$task_data.appendChild(n),
		t.$marker_area = n,
		t._markerRenderer = t._task_renderer("markers", e, t.$marker_area, null);
	}),
	...
	t.addMarker = function (t) {
		return t.id = t.id || dhtmlx.uid(),
		this._markers[t.id] = t,
		t.id
	},
	...
	t.renderMarkers = function () {
		...
		return this._markerRenderer.render_items(t),
		!0
	}
});
